...
class AssociationFiller:
    ...
    def __call__(self, image, anns, meta):
        ...
        # LOG.info('valid area: %s', valid_area)
        ...
        # LOG.info(f'Reg_L: {self.fields_reg_l}')
        # LOG.info(f'Reg_L Shape: {self.fields_reg_l.shape}')
        ...
        # LOG.info(f'fill_values: {len(fill_values)}, ref_fields: {self.fields_reg_l.shape}, fill: {self.fill(keypoint_sets, fill_values)}, kp_sets: {len(keypoint_sets)}, fields: {fields.shape}')
    ...
    def fill_keypoints(self, keypoints, fill_values):
        for field_i, joint1i, joint2i in self.config.fill_plan:
            ...
            # LOG.info('fov check: j1 = %s, j2 = %s', joint1, joint2)
            out_field_of_view_1 = (
                joint1[0] < 0
                or joint1[1] < 0
    ...
    def fill_association(self, field_i, joint1, joint2, fill_values):
        # First calculate the offset vector offset between joint1 and joint2, and dynamically generate a matrix sink according to the modulus length of the offset.

        # These are used to create the area that is used as high confidence for PAF connections where it samples along the 
        # line between two joints and deals with some corner cases like when joints are being very close to each other offset between joints
        offset = joint2[:2] - joint1[:2]
        offset_d = np.linalg.norm(offset)

        # LOG.info({f'Offset: {offset_d}'})
        # LOG.info({f'Offset Shape: {offset_d.shape}'})

        # outputs  a (2, s, s) matrix the content inside is the s * s size, 
        # which is the offset value in the x direction and y direction.

        # dynamically create s, where min value set by the authors is 3
        ...
        # set fields
        # since a point cannot be selected as the intermediate node that can point to joint1 and joint2, the number of intermediate
        # points is dynamically set bellow, where through 'num' a point is actually taken between joint1 and joint2 to represent a 
        # connection between joint1 and joint2
        ...
        for f in frange:
            for xyo in xyv:
                ...
                # convert to hashable coordinate and check if
                ...
                if sink_l > self.fields_reg_l[field_i, fij[1], fij[0]]:
                    continue
                # give priority to the closest joint
