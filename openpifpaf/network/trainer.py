...
class Trainer():
    ...
    def __init__(self, model, loss, optimizer, out, *,
        ...
        # LOG.info('Model: %s , Loss: %s , Optim: %s , Out: %s , checkpoint_shell: %s , lr_sched: %s , device: %s , model_meta_data: %s', 
        # self.model,self.loss, self.optimizer , self.out , self.checkpoint_shell , self.lr_scheduler , self.device, self.model_meta_data)
    ...
    def train(self, scenes, epoch):
        ...
        for batch_idx, (data, target, _) in enumerate(scenes):
            ...
            if batch_idx % self.log_interval == 0:
                ...
                # writer.add_scalar("Loss", batch_info['loss'], batch_idx)
                # writer.add_scalar("Head losses", batch_info['head_losses'][0], batch_idx)
